function [I, xcut,ycut,l,m ] = optim2d( X ,type , threshold)
...
%
%
%This algorithm is a brute force algorithm, in the sense that proves each
%possible partition of the plane in 4 regions. 
%
%Input:
...
for i=1:nx
    for j=1:ny
        ...
        %clockwise
        r1=X(X(:,1)>=Sx(i) & X(:,2)>=Sy(j),:);
        r2=X(X(:,1)>Sx(i) & X(:,2)<Sy(j),:);
        r3=X(X(:,1)<=Sx(i) & X(:,2)<=Sy(j),:);
        r4=X(X(:,1)<Sx(i) & X(:,2)>Sy(j),:);
              
        %Computation of weights according to exogenous distribution Z which
        %contains X.
        w1=Z(Z(:,1)>=Sx(i) & Z(:,2)>=Sy(j),:);
        w2=Z(Z(:,1)>Sx(i) & Z(:,2)<Sy(j),:);
        w3=Z(Z(:,1)<=Sx(i) & Z(:,2)<=Sy(j),:);
        w4=Z(Z(:,1)<Sx(i) & Z(:,2)>Sy(j),:);
