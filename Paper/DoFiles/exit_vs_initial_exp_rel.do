...
*Winzorise all at 90th percentile
for var time_s time_e amount_s amount_e: capture egen X90 = pctile(X) , p(90)
for var time_s time_e amount_s amount_e: ///
	 replace X=X90 if X>X90 & X~=.
drop *90
...
esttab using "$sharelatex\Tables\reg_results\exit_vs_initial_exp.csv", se star(* 0.1 ** 0.05 *** 0.01)  ///
