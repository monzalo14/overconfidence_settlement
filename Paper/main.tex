...
\date{This draft:  20 January, 2017 \\[2 cm]}
...
\begin{table}[H]
    ...
    \label{Sum_Stats}
    \begin{center}
       \scriptsize{\input{./Tables/SS_bylawyer.tex}}
    \end{center}
    \footnotesize
    \textit{Notes:} 
    Summary stats of outcomes, basic and strategic variables by type of (employee's) lawyer.
...
\begin{table}[H]
    ...
    \footnotesize    
    \textit{Notes:} 
    Summary stats of beliefs, total \& by type of (employee's) lawyer. First column displays beliefs without any conditioning, second columns conditions on knowing type of lawyer. Third and fourth columns of Panel B and C are condition on employee answering the survey so that comparison across is allowed.
...
\end{center}
 \footnotesize
\textit{Notes:} 
Omited variable for \emph{education} is elementary education, while for \emph{Mistreated} is very angry, and the scale is increasing. We use a measure of relative overconvidence which is the \textit{percentage} quantity expected over the calculators prediction.
...
\begin{figure}[H]
    ...
    \begin{center}
        \includegraphics[width=0.7\textwidth]{Amountasked.png}
        \end{center}
    \footnotesize \textit{Notes: This graph shows the average amount (in thousand pesos) asked, won, minimum legal compensation, and minimum legal compensation plus lost wages, by outcome. The bottom legend shows the percentage of cases with each outcome for our database (N=5005). Note that, by definition, workers have no chance of recovery in drops and expiries. In our data, all settlements imply a positive recovery for the worker; finally, \textbf{for judgments, only 24\% of the time does the worker recover anything.}} 
    {\footnotesize \textit{Script: } \texttt{Amounts.Rmd}}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{center}
        ...
        {\footnotesize \textit{Do file: } \texttt{con\_overtime.do}}
\end{figure}
...
%#appendix
\begin{table}[H]
    \caption{Treatment Effects by Month (including basic variables controls)}
    \label{Table_effects}
    \begin{center}
    \scriptsize{\input{./Tables/Minicortes_2_B_PA.tex}}
    \end{center}
    \footnotesize
    \textit{Notes:} 
    Calc=Conc and Calc=Conc=0 shows the p-value for the respective tests.
